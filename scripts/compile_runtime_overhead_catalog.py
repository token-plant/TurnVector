...
def bounded_sum(values, label: str) -> int:
    ...
    for value in values:
        ...
        total += value
    return total
def physical_catalog_identity(executable: dict, logical: dict) -> None:
    top = {"artifact", "architecture", "byte_length", "catalog_section", "cpu_subtype", "cpu_type", "fixup_proof", "format", "header", "loader_commands", "present", "sections", "segments", "sha256"}; exact(executable, top, "Core executable identity")
    physical, proof, header, loader, code, segments = (executable[name] for name in ("catalog_section", "fixup_proof", "header", "loader_commands", "sections", "segments")); section_keys = {"address", "alignment", "byte_length", "file_offset", "flags", "relocation_count", "relocation_offset", "reserved1", "reserved2", "reserved3", "section", "segment"}; segment_keys = {"file_offset", "file_size", "flags", "initial_protection", "max_protection", "segment", "vm_address", "vm_size"}
    exact(physical, section_keys, "Core physical Catalog section"); exact(proof, {"algorithm", "bind_count", "catalog_target_count", "chained_fixups", "linkedit", "linkedit_sources", "rebase_count", "sha256", "streams"}, "Core fixup proof"); exact(header, {"file_type", "flags"}, "Core executable header")
    if executable["artifact"] != "turnvector-daemon" or executable["format"] != "mach_o_64" or executable["present"] is not True or header["file_type"] != 2 or type(header["flags"]) is not int or executable["byte_length"] != sum(item.get("byte_length", -1) for item in code): raise ValueError("unsupported Core executable identity")
    exact(loader, {"dependent_dylibs", "dynamic_linker", "entry_point", "normalized_sha256"}, "Core loader commands"); exact(loader["entry_point"], {"file_offset", "stack_size"}, "Core entry point"); hex_identity(loader["normalized_sha256"], "Core load commands")
    if type(loader["dynamic_linker"]) is not str or not loader["dynamic_linker"].startswith("/") or type(loader["entry_point"]["file_offset"]) is not int or type(loader["entry_point"]["stack_size"]) is not int or min(loader["entry_point"].values()) < 0 or not loader["dependent_dylibs"]: raise ValueError("invalid Core loader commands")
    for item in loader["dependent_dylibs"]:
        exact(item, {"compatibility_version", "current_version", "path", "timestamp"}, "Core dependent library")
        if type(item["path"]) is not str or not item["path"].startswith("/") or any(type(item[name]) is not int or item[name] < 0 or item[name] > (1 << 32) - 1 for name in ("compatibility_version", "current_version", "timestamp")): raise ValueError("invalid Core dependent library")
    for item in segments:
        exact(item, segment_keys, "Core segment")
        if any(type(item[name]) is not int or item[name] < 0 or item[name] > (1 << 64) - 1 for name in ("file_offset", "file_size", "vm_address", "vm_size")) or item["file_offset"] + item["file_size"] > 1 << 64 or item["vm_address"] + item["vm_size"] > 1 << 64 or not item["vm_size"] or item["segment"] != "__PAGEZERO" and not item["file_size"]: raise ValueError("invalid Core segment range")
    names = [item["segment"] for item in segments]; expected_segments = {"__PAGEZERO": (0, 0, 0), "__TEXT": (5, 5, 0), "__DATA_CONST": (3, 3, 0x10), "__DATA": (3, 3, 0)}
    if names != list(expected_segments) or any((item["max_protection"], item["initial_protection"], item["flags"]) != expected_segments[item["segment"]] or item["segment"] == "__PAGEZERO" and (item["file_offset"] or item["file_size"] or item["vm_address"]) for item in segments) or segments[1]["file_offset"] != 0: raise ValueError("unsupported Core segment topology")
    overlap = lambda a, an, b, bn: max(a, b) < min(a + an, b + bn); owner = [item for item in segments if physical["file_offset"] >= item["file_offset"] and physical["file_offset"] + physical["byte_length"] <= item["file_offset"] + item["file_size"] and physical["address"] >= item["vm_address"] and physical["address"] + physical["byte_length"] <= item["vm_address"] + item["vm_size"]]
    for index, left in enumerate(segments):
        for right in segments[index + 1:]:
            if left["file_size"] and right["file_size"] and overlap(left["file_offset"], left["file_size"], right["file_offset"], right["file_size"]) or overlap(left["vm_address"], left["vm_size"], right["vm_address"], right["vm_size"]): raise ValueError("overlapping Core segment topology")
    zero = ("flags", "relocation_count", "relocation_offset", "reserved1", "reserved2", "reserved3")
    if len(owner) != 1 or owner[0]["segment"] != "__DATA_CONST" or physical["address"] - owner[0]["vm_address"] != physical["file_offset"] - owner[0]["file_offset"] or (physical["segment"], physical["section"], physical["byte_length"], physical["alignment"]) != (logical["segment"], logical["section"], logical["byte_length"], 0) or any(physical[name] for name in zero): raise ValueError("Core Catalog physical binding mismatch")
    code_ranges = []
    for item in code:
        exact(item, section_keys | {"sha256"}, "Core instruction section"); hex_identity(item["sha256"], "Core instruction section")
        if any(type(item[name]) is not int or item[name] < 0 or item[name] > (1 << 64) - 1 for name in section_keys - {"section", "segment"}) or not item["byte_length"] or not item["flags"] & (0x80000000 | 0x400): raise ValueError("invalid Core instruction section")
        owners = [segment for segment in segments if item["file_offset"] >= segment["file_offset"] and item["file_offset"] + item["byte_length"] <= segment["file_offset"] + segment["file_size"] and item["address"] >= segment["vm_address"] and item["address"] + item["byte_length"] <= segment["vm_address"] + segment["vm_size"]]
        if len(owners) != 1 or owners[0]["segment"] != item["segment"] or (owners[0]["max_protection"], owners[0]["initial_protection"]) != (5, 5) or item["address"] - owners[0]["vm_address"] != item["file_offset"] - owners[0]["file_offset"] or overlap(item["file_offset"], item["byte_length"], physical["file_offset"], physical["byte_length"]) or overlap(item["address"], item["byte_length"], physical["address"], physical["byte_length"]): raise ValueError("Core instruction section is not uniquely RX and disjoint from Catalog")
        code_ranges.append(item)
    if len({(item["segment"], item["section"]) for item in code}) != len(code) or any(overlap(left["file_offset"], left["byte_length"], right["file_offset"], right["byte_length"]) or overlap(left["address"], left["byte_length"], right["address"], right["byte_length"]) for index, left in enumerate(code_ranges) for right in code_ranges[index + 1:]) or not any(item["file_offset"] <= loader["entry_point"]["file_offset"] < item["file_offset"] + item["byte_length"] for item in code): raise ValueError("invalid Core instruction layout or entry point")
    exact(proof["linkedit"], {"file_offset", "flags", "initial_protection", "max_protection", "segment", "vm_address"}, "Core linkedit proof"); linkedit = proof["linkedit"]
    if type(linkedit["file_offset"]) is not int or type(linkedit["vm_address"]) is not int or linkedit["file_offset"] < 0 or linkedit["vm_address"] < 0 or linkedit != {"file_offset": linkedit["file_offset"], "flags": 0, "initial_protection": 1, "max_protection": 1, "segment": "__LINKEDIT", "vm_address": linkedit["vm_address"]} or physical["file_offset"] + physical["byte_length"] > linkedit["file_offset"] or physical["address"] + physical["byte_length"] > linkedit["vm_address"]: raise ValueError("invalid Core linkedit proof")
    records = proof["linkedit_sources"]
    for item in [*proof["streams"], *records]:
        exact(item, {"byte_length", "file_offset", "kind", "sha256"}, "Core linkedit source"); hex_identity(item["sha256"], "Core linkedit source")
        if type(item["byte_length"]) is not int or type(item["file_offset"]) is not int or item["byte_length"] < 0 or item["file_offset"] < 0: raise ValueError("invalid Core linkedit source")
    if records != sorted(records, key=canonical) or len({canonical(item) for item in records}) != len(records) or any(item["file_offset"] < linkedit["file_offset"] for item in records) or any(overlap(left["file_offset"], left["byte_length"], right["file_offset"], right["byte_length"]) for index, left in enumerate(records) for right in records[index + 1:]) or [item["kind"] for item in proof["streams"]] != ["rebase", "bind", "weak_bind", "lazy_bind", "export"] or any(item["byte_length"] and item not in records or not item["byte_length"] and item["file_offset"] for item in proof["streams"]) or proof["algorithm"] != "classic_dyld_info_loader_state_v2" or proof["chained_fixups"] is not False or proof["catalog_target_count"] != 0 or any(type(proof[name]) is not int or proof[name] < 0 for name in ("bind_count", "rebase_count")): raise ValueError("unsupported Core fixup proof")
    hex_identity(proof["sha256"], "Core fixup destinations"); identity = {name: executable[name] for name in ("catalog_section", "fixup_proof", "header", "loader_commands", "sections", "segments")}
    if executable["sha256"] != hashlib.sha256(canonical(identity)).hexdigest(): raise ValueError("Core executable identity hash mismatch")
def core_identity(directory: Path) -> tuple:
    ...
    support, catalog, event, cardinality, carry, sections = (descriptor[name] for name in ("support", "catalog", "event_registry", "cardinality_inputs", "prepared_carry", "section_identities"))
    capacity, frame, lookup, work = (catalog[name] for name in ("capacity", "frame", "lookup", "worst_case_work")); funding, vector, start, records = (support[name] for name in ("funding_claim", "outstanding_credit_vector", "start_count", "records"))
    exact(catalog, {"schema_version", "capacity", "frame", "lookup", "worst_case_work"}, "Core Catalog"); exact(capacity, {"max_canonical_bytes", "max_entries", "max_entry_bytes"}, "Core Catalog capacity"); exact(frame, {"encoding", "header_bytes", "section_bytes"}, "Core Catalog frame"); exact(lookup, {"algorithm", "key_bytes"}, "Core Catalog lookup"); exact(work, {"entry_bytes_validated", "key_bytes_compared", "key_comparisons"}, "Core Catalog work")
    exact(sections, {"catalog_payload", "executable_text", "native_text"}, "Core section identities"); catalog_section = sections["catalog_payload"]; expected_section = {"byte_length": frame["section_bytes"], "encoding": frame["encoding"], "executable": False, "format": "mach_o_64", "header_bytes": frame["header_bytes"], "padding_byte": 0, "section": "__tvcatalog", "segment": "__DATA_CONST", "writable": False}; exact(catalog_section, set(expected_section), "Core Catalog physical section"); physical_catalog_identity(sections["executable_text"], catalog_section)
    ...
    for value in (*capacity.values(), frame["header_bytes"], frame["section_bytes"], event["mandatory_crossing_max"], event["max_entries"], event["max_kinds"], carry["mandatory_suballocation_max"], carry["safety_suballocation_max"], funding["max_claims_per_obligation"], vector["max_dimensions"], *records.values(), start["max_cells"], start["max_horizons"], start["max_physical_credits"]): positive(value, "Core Catalog capacity", (1 << 32) - 1)
    ...
    if (any(type(version) is not int or version != 1 for version in schemas) or cardinality["encoding"] != "u16" or support["operations"] != list(SUPPORT) or support["pools"] != list(POOLS) or
            ...
            frame != {"encoding": "fixed_binary_tuple_v1", "header_bytes": 256, "section_bytes": 4194560} or capacity["max_canonical_bytes"] != frame["section_bytes"] - frame["header_bytes"] or canonical(catalog_section) != canonical(expected_section) or lookup != {"algorithm": "sorted_sha256_key_binary_search_then_canonical_equality_v1", "key_bytes": 32} or work != {"entry_bytes_validated": capacity["max_entry_bytes"], "key_bytes_compared": 32 * comparisons, "key_comparisons": comparisons} or
