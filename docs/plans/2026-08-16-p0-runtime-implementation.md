...
Generation through C12; every unrelated invalidated request uses the same owner.
...
Catalog/outer identities; C15 owns non-support admitted resource capacity, while C07,
C08, C16, C17, and C18 incrementally implement one Core-owned support ledger over active charges, unstarted
...
| D06 | `docs(architecture): define exact execution profiles` | Amend ADR 0031 and the glossary to define Certified Execution Profile and Execution Route Identity, distinguish stable exact identity from dynamic Resource Evidence, and bind every graph, memory, kernel/fusion, KV, speculative, prefix, and command-replay route change to a new exact Capability Key | Exact versus one-field-drift matrix, explicit `NONE` optimization members, no Serving Profile collision, runtime range inference, favorable-resource authorization, raw GPU-address promise, or P-1 status upgrade | docs only |
...
| C08 | `feat(core): reserve ordinary and lifecycle support` | Extend the sole ledger with atomic optional Ordinary Reservation Claim/obligation/credit/active-record creation plus pre-trigger post-load/post-observation description and first/next safety lifecycle reserves; every lifecycle obligation enters `pending` only when its reserved trigger occurs or closes on typed impossibility, and no trigger allocates capacity | Optional nonempty claim/type/scope, ordinary exhaustion before Effect, description and safety maxima, load/observation/sample success/failure/cancel, first pre-ready and next-before-expiry reserve, pending/begin/impossible-close, mandatory/safety nonborrowability, stable generation | <= 400 |
...
| E24 | `test(runtime): enforce backend conformance fixtures` | Shared Fake runner for Bootstrap/initialization/Capability/Generation Semantics/Signal/Operation descriptors, Catalog/Lifecycle qualification, post-load Model Descriptor equality and Request Description refresh, full support envelopes/lifecycle states, ownership, CAS, transitions, release, and shutdown | Trusted semantic/build/Catalog identities, call order, pre-ready/recovery/drain no-call matrix, local-stale independence, release-before-unload, empty-handle-only raw deallocation, Core-owned sliding budget and watchdog split, result-gated Clean Shutdown Boundary/fail-stop | <= 400 |

### P0-3: Resource Governor And Residency

| ID | Commit subject | Behavior slice | Required verification | Target LOC |
|---|---|---|---|---:|
| G01 | `feat(resources): sequence backend resource samples` | Sole safety-trigger owner over Signal Contract-bound allocator/cache provenance and freshness: after E17/C08's first obligation, request the next C08 safety-sample obligation from the pre-reserved nonborrowable schedule before evidence expiry, then consume only its typed Result; owns no credit/ledger mutation | First pre-ready and later normal/degraded/recovery/drain samples; exact trigger/obligation/result identity, early/duplicate/stale/missing, shutdown-impossible close, no post-expiry allocation, no ordinary borrowing, and contract mismatch | <= 380 |
...
| G08 | `feat(governor): reserve residency before load` | Separate Model Descriptor-based Residency Reservation plus requirement for C08's bounded post-load Model/Request-description obligation set before any load Effect | No load without both reservations; exact waiter/model bound; cancel-before-start makes no Backend/description call and rolls both back | <= 400 |
| G09 | `feat(runtime): coordinate residency demands` | Shared loads/waiters/FIFO/timeout/rollback; before load, obtain C12's bounded revalidation scope and C08's nonborrowable operation-obligation set; failure/cancel closes it, successful load advances Generation and marks the exact set pending for C12 | Resident cross-model reissue, exact post-load model+request set/credits, unrelated Warming defer, current loaded waiters before Admission, failure/impossible close, no post-success allocation, drift scopes Unavailable | <= 400 |
...
exported graph plus one preallocated memory arena with stable offsets; kernel
bundle, fusion plan,
PagedKV, Speculative Decode, Prefix Reuse, and command replay are explicit
...
| N04 | `build(native): define the exported graph abi` | Canonical exact-shape signatures, logits/new-KV outputs, artifact manifest/import contract, and baseline Execution Route descriptor binding the exported graph and fixed memory plan with every deferred optimization set to explicit `NONE` | Double-export and Route-descriptor byte identity, Python round-trip harness, and one-member drift changes Route identity | <= 400 |
...
| N10 | `feat(native): describe model registrations` | Generate versioned conservative Model Descriptor including nonzero `u32` vocabulary size and exact Execution Route Identity from the registered graph manifest/capability before registration and after load | Nonresident operation, field/identity/hash/Route post-load equality, drift marks Revision Unavailable | <= 400 |
...
| N13 | `feat(native): describe token requests` | Stateless validation of frozen Token Request plus Model Descriptor/current Backend Generation and finite route-bearing Capability Requirement Set generation | Initial/post-load generation binding and complete phase/batch/Shape/Route matrix; no Core refresh, Envelope, registry, or allocation | <= 400 |
